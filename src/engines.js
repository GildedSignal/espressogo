...
export async function attachEngines(game) {
  for (const [id, config] of Object.entries(ENGINES)) {
    game.registerOpponent(id, { label: config.label, adapter: null });
  }
  const loaded = {};
  for (const [id, config] of Object.entries(ENGINES)) {
    try {
      loaded[id] = await createEngine(config);
      game.registerOpponent(id, { label: config.label, adapter: loaded[id] });
    } catch (error) {
      console.warn(`Could not load ${config.label}`, error);
      game.registerOpponent(id, { label: `${config.label} (unavailable)`, adapter: null });
    }
  }
  if (loaded.espresso) game.setOpponent(loaded.espresso.requestMove);
}
